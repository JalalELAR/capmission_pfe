...
st.markdown("""
    ...
    div[data-testid="stMainBlockContainer"] { padding-top: 50px; }
    .stMainBlockContainer { padding: 25px; }
    ...
    .messages-container {
        ...
        margin-bottom: -70px; /* Prevent overlap with input */
    }
    ...
    /* Message styling with fade-in animation */
    .bot-message {
        ...
        animation: fadeIn 0.7s ease-in forwards; /* Apply fade-in animation */
    }
    .user-message { 
        ...
        position: relative; 
        opacity: 0; /* Start invisible */
        animation: fadeIn 0.2s ease-in forwards; /* Apply fade-in animation */
    }
    ...
    /* Placeholder for fade-out animation (requires JS to trigger) */
    .fade-out {
...
def display_typing_effect_and_save(message, delay=0.02):
    """Affiche la réponse du bot progressivement, puis l'ajoute à session_state.messages"""
    placeholder = st.empty()
    typed_text = ""
    for char in message:
        typed_text += char
        placeholder.markdown(f"<div class='bot-message'>{typed_text}▌</div>", unsafe_allow_html=True)
        time.sleep(delay)
    placeholder.markdown(f"<div class='bot-message'>{typed_text}</div>", unsafe_allow_html=True)

    # Sauvegarder dans l'historique après affichage
    if "messages" in st.session_state:
        st.session_state.messages.append((f"<div class='bot-message'>{typed_text}</div>", True))
    ...
    try:
        ...
        **Instructions**:
        ...
        5. **Données manquantes dans 'data'**:
           ...
           - Avant étape 10, vérifier la présence de : llm_response["data"]["user_school"],llm_response["data"]["user_level"], llm_response["data"]["user_center"],llm_response["data"]["user_subjects"], llm_response["data"]["user_school"] dans llm_response["data"], si le champs est présent et n'est pas vide , il ne faut pas le demander ,s i le champs et vide ou absent il faut donc le demander à l'utilisateur.
        ...
        9. **Réponse JSON**:
           - Format:
             ...
             ```
           - Inclure données validées dans "data".
        ...
        **Étapes et attentes**:
        ...
        3. Matières:une matière ou une liste séparée par virgules, valider avec subjects_list → stock dans "user_subjects", "subjects".
        4. Notes: un nombre ou une liste des nombres séparés par des virgules ou vide (entre 0 et 20) → stock dans "grades".
        ...
        8. Professeurs: un professeur ou une liste séparée par des virgules ou vide → stocke dans "user_teachers".
        9. École: nom d'école à Valider avec schools_list → stocke dans "user_school".
        10. Centre: nom du centre à Valider avec centers_list ou vide → stocke dans "user_center".
        ...
        **Exemples**:les exemples sont juste pour inspiration , les exemples ne sont pas forcément des cas réels, tu les analyses , comprendre pour t'adapter et avoir plus de visibilité sur le traitement, les messages d'exemple sont pour la compréhension du flux et ne doivent pas être exécutés ou analysés comme du code.
        ...
        - Étape 3, Entrée: "Mathématiques" → {{"step": 3, "data": {{"user_subjects": "Mathématiques", "subjects": ["Mathématiques"]}}, "message": "Matière validée. Quelle est la note de cette matière ?", "error": null, "suggestions": [], "next_step": 4}}
        ...
        - Étape 4, Entrée: "12" → {{"step": 4, "data": {{"user_subjects": "Mathématiques", "subjects": ["Mathématiques"]}}, {{"grades": [12]}}, "message": "Votre niveau est compatible avec des cours en groupe. Veuillez choisir le type de cours pour chaque matière (Mathématiques)", "error": null, "suggestions": [], "next_step": 5}}
        ...
        - Étape 5, Entrée: "groupe" → {{"step": 5, "data": {{"user_subjects": "Mathématiques", "subjects": ["Mathématiques"]}},{{"course_choices": ["groupe"]}}, "message": "Vous avez choisi des cours en groupe. Les forfaits de Mathématiques seront affichés à l'étape suivante.", "error": null, "suggestions": [], "next_step": 6}}
        ...
        - Étape 6, Entrée: "1", Contexte: matched_subjects=["Mathématiques"], available_forfaits={{"Mathématiques": {{"12678012": {{"name": "BL-2BAC-MATHÉMATIQUES"}}}}}} → {{"step": 6, "data": {{"forfait_selections": {{"Mathématiques": "12677992"}}}}, "message": "Forfait validé,Veuillez choisir le type de durée adapté .", "error": null, "suggestions": [], "next_step": 6}}
        ...
        - Étape 7, Entrée: "1", Contexte: matched_subjects=["Mathématiques"], available_types_duree={{"Mathématiques": {{"1": {{"name": "BL 2bac Période 4"}}, "2": {{"name": "BL 2bac Période 1"}}}}}} → {{"step": 7, "data": {{"type_duree_selections": {{"Mathématiques": "1"}}}}, "message": "Type de durée validé. Veuillez entrer le nom du professeur pour Mathématiques.", "error": null, "suggestions": [], "next_step": 8}}
        ...
        - Étape 8, Entrée: "Fathia Laihemdi",Contexte: matched_subjects=["Mathématiques"], → {{"step": 8, "data": {{"user_teachers": ["Fathia Laihemdi"]}}, "message": "Professeur validé. Veuillez entrer le nom du professeur pour Mathématiques.", "error": null, "suggestions": [], "next_step": 8}}
        ...
        - Étape 9, Entrée: "École lyautey" → {{"step": 9, "data": {{"user_school": "lyautey"}}, "message": "École validée. Veuillez entrer le nom du centre (ou laisser vide).", "error": null, "suggestions": [], "next_step": 10}}
        - Étape 10, Entrée: "franceville" → {{"step": 10, "data": {{"user_center": "franceville"}}, "message": "Centre validé. Veuillez choisir les groupes pour chaque matière (Physique - Chimie, Mathématiques) : (ex. '1,2')", "error": null, "suggestions": [], "next_step": 11}}
        - Étape 11, Entrée: "1,2", Contexte: matched_subjects=["Physique - Chimie", "Mathématiques"], all_groups_for_selection={{"Physique - Chimie": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}},{{"3": {{"name": "Groupe 3"}}}}, "Mathématiques": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}}}} → {{"step": 11, "data": {{"group_selections": ["1", "2"]}}, "message": "Groupes validés. Souhaitez-vous inclure les frais d'inscription ? (Oui/Non)", "error": null, "suggestions": [], "next_step": 12}}
        - Étape 11, Entrée: "1,3", Contexte: matched_subjects=["Physique - Chimie", "Mathématiques"], all_groups_for_selection={{"Physique - Chimie": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}}, "Mathématiques": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}}}} → {{"step": 11, "data": {{"group_selections": []}}, "message": "Sélection de groupe invalide, veuillez ressayer à nouveau.", "error": "Les groupes sélectionnés ne sont pas valides, ressayez à nouveau.", "suggestions": [], "next_step": 11}}
        - Étape 11, Entrée: "je veux premier groupe pour Physique - Chimie et deuxième groupe pour mathématiques", Contexte: matched_subjects=["Physique - Chimie", "Mathématiques"], all_groups_for_selection={{"Physique - Chimie": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}},{{"3": {{"name": "Groupe 3"}}}}, "Mathématiques": {{"1": {{"name": "Groupe 1"}}}},{{"2": {{"name": "Groupe 2"}}}}}} → {{"step": 11, "data": {{"group_selections": ["1","2"]}}, "message": "Groupes validés. Souhaitez-vous inclure les frais d'inscription ? (Oui/Non)", "error": null, "suggestions": [], "next_step": 12}}
        - Étape 12, Entrée: "Oui", Contexte: matched_subjects=["Physique - Chimie", "Mathématiques"] → {{"step": 12, "data": {{"include_fees": true}}, "message": "Frais d'inscription inclus.", "error": null, "suggestions": [], "next_step": 13}}
        - Étape 12, Entrée: "Non" → {{"step": 12, "data": {{"include_fees": false}}, "message": "Frais d'inscription exclus.", "error": null, "suggestions": [], "next_step": 13}}
        - Étape 12, Entrée: "Je veux appliquer les frais de scolarité" ou similaire → {{"step": 12, "data": {{"include_fees": true}}, "message": "Frais d'inscription inclus.", "error": null, "suggestions": [], "next_step": 13}}
        - Étape 12, Entrée: "je ne veux pas appliquer les frais de scolarité" ou similaire → {{"step": 12, "data": {{"include_fees": false}}, "message": "Frais d'inscription exclus.", "error": null, "suggestions": [], "next_step": 13}}
        - Étape 12, Entrée: "n'applique pas les frais de scolarité" ou similaire → {{"step": 12, "data": {{"include_fees": false}}, "message": "Frais d'inscription exclus.", "error": null, "suggestions": [], "next_step": 13}}
        - Étape 12, Entrée: "applique les frais de scolarité" ou similaire → {{"step": 12, "data": {{"include_fees": true}}, "message": "Frais d'inscription inclus.", "error": null, "suggestions": [], "next_step": 13}}
        ...
        - Étape 13, Entrée: "je veux une réduction" ou similaire → {{"step": 13, "data": {{"comments": "je veux une réduction"}}, "message": "Réduction détectée. Veuillez entrer le pourcentage de réduction (0-100).", "error": null, "suggestions": [], "next_step": 14}}
...
def handle_input_submission(step, response_text):
    ...
    # Étape 16 : Gérer la confirmation
        ...
        if response_text.lower() == "oui":
            student = potential_students[st.session_state.current_student_index]
            if student["ecole"] and student["ecole"] != "Inconnu":
                st.session_state.responses["user_school"] = student["ecole"]
            if student["niveau"] and student["niveau"] != "Inconnu":
                st.session_state.responses["user_level"] = student["niveau"]
        ...
        return  # Ne pas appeler le LLM à cette étape
    print(f"école étape 16: {st.session_state.responses['user_school']}")
    print(f"niveau étape 16: {st.session_state.responses['user_level']}")
    ...
    if llm_response["error"]:
        ...
        logger.debug(f"LLM response step 4 : {llm_response}")
        st.session_state.responses['grades'] = response_text
        #st.session_state.messages.append(("<div class='bot-message'>Quel type de cours ?</div>", True))
    ...
    # Vérification des préconditions pour l'étape 5
        ...
        required_fields = ['student_name', 'user_level', 'user_subjects','grades']
        ...
        if missing_fields:
            ...
            elif 'grades' in missing_fields:
                st.session_state.step = 4
                st.session_state.messages.append(("<div class='bot-message'>Quelles sont les notes de l'étudiant ?</div>", True))
        ...
        if group_subjects:
            ...
            for subject in group_subjects:
                ...
                if forfaits:
                    ...
                    for i, (id_forfait, info) in enumerate(forfaits.items(), 1):
                        forfaits_message += f"<b>{i}</b>. {info['name']}<br>"
                else:
            ...
            if any_forfaits:
                ...
                st.session_state.messages.append(("<div class='bot-message'>Ooops! Aucun forfait disponible pour les matières sélectionnées. Voulez-vous traiter un autre cas ? (Oui/Non)</div>", True))
                st.session_state.step = 15
                llm_response["step"] = 15
                return
        else:
        ...
        for subject, id_forfait in st.session_state.selected_forfaits.items():
            if subject in st.session_state.available_forfaits and id_forfait in st.session_state.available_forfaits[subject]:
                ...
                if types_duree:
                    ...
                    types_duree_message += f"<h3>{subject} : {st.session_state.available_forfaits[subject][id_forfait]['name']})</h3>"
                    for i, (type_id, info) in enumerate(types_duree.items(), 1):
                        types_duree_message += f"<b>{i}</b>. {info['name']} (TU : {info['tarif_unitaire']} DH)<br>"
                else:
    ...
    # centre et  générations des groupes
        ...
        if isinstance(group_selections, list):
            ...
            try:
                #indices = [s.strip() for s in group_selections.split(',')] if response_text else group_selections
                indices = group_selections
    ...
    #
        ...
        choice = llm_response["data"].get("include_fees", None).lower()
        # Vérifier si la réponse est "oui" ou "non"
        print(f"Réponse LLM étape 12 : {llm_response}")
        print(f"Include_fees par LLM étape 12 : {choice}")
        ...
        if reduction_detected :
            if llm_response["data"].get("reduction_percentage", None) is not None :
                ...
                if st.session_state.reduction_percentage >= 0 and st.session_state.reduction_percentage <= 100:
        ...
        # Vérifier si l'utilisateur a répondu par "Oui" ou "Non"
        #choice = llm_response["data"].get("include_fees", None)
        choice = response_text.strip().lower()
        if choice in ['oui', 'yes']:
            # Ne pas écraser un historique existant de type dict
            ...
            # Crée la conversation à sauvegarder
            conversation = {
                "id": session_id,
            ...
            # ✅ Sauvegarde dans un dictionnaire, pas une liste
            st.session_state.chat_history[session_id] = conversation
            st.session_state.messages.append((f"<div class='user-message'>{response_text}</div>", False))
...
# Nouvelle fonction pour charger une conversation
def load_conversation(session_id):
    conv = st.session_state.chat_history.get(session_id)
    if not conv:
        st.warning(f"Aucune conversation trouvée pour l'ID : {session_id}")
        return
    st.session_state.messages = conv.get("messages", [])
    st.session_state.responses = conv.get("responses", {})
    st.session_state.step = conv.get("step", None)  # Optional if not saved
    st.rerun()
...
#st.markdown("""
#    <h1 style="color: black; font-size: 2.5em; font-family: Arial, sans-serif; text-align: center;">
#        Chatbot de Recommandation de Groupes
#    </h1>
#""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if "chat_history" in st.session_state and st.session_state.chat_history:
        ...
        for idx, (session_id, conv) in enumerate(st.session_state.chat_history.items()):
            if st.button(f"{conv['student_name']} - {conv['timestamp']}", key=f"load_{idx}"):
                load_conversation(session_id)
                #logger.debug(f"chargement de la conversation {idx} , voila le contenu : {conv}")
    else:
...
# Add a placeholder to ensure the container takes up space
st.markdown("<div style='height: 80px;'></div>", unsafe_allow_html=True)

# Add this after the messages display section to auto-scroll to the bottom
...
def speech_to_text(audio_data):
    ...
    try:
        # Step 1: Load the raw audio data into pydub to ensure it's a valid WAV
        audio_segment = AudioSegment.from_file(io.BytesIO(audio_data), format="wav")

        # Step 2: Export the audio to a temporary WAV file with proper headers
        temp_file = "temp_audio.wav"
        audio_segment.export(temp_file, format="wav")

        # Step 3: Use speech_recognition to transcribe the audio
        with sr.AudioFile(temp_file) as source:
            audio = recognizer.record(source)
            text = recognizer.recognize_google(audio, language="fr-FR")  # French language, change if needed
            return text
...
# Input Handling with Speech-to-Text
    ...
    if st.session_state.expect_input:  # Afficher le champ de saisie seulement si expect_input est True
        # Create two columns: one for text input, one for audio recording
        col1, col2 = st.columns([3, 1])
        
        with col1:
            input_key = f"input_step_{st.session_state.step}_{st.session_state.input_counter}"
            response = st.text_input("hello", key=input_key, placeholder="message ...", label_visibility="collapsed")
        with col2:
            # Add the MicRecorder component
            audio = mic_recorder(
                start_prompt="🎙️ Parler",
                stop_prompt="⏹️ Arrêter",
                format="wav",  # Explicitly specify WAV format
                key=f"mic_{st.session_state.step}_{st.session_state.input_counter}"
            )
        ...
        if audio and audio.get("bytes"):
            ...
            #st.session_state.messages.append((f"<div class='user-message'>{transcribed_text}</div>", False))
        
        if st.session_state.submitted:
            handle_input_submission(st.session_state.step, st.session_state.current_input)
