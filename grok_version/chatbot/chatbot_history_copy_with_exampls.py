    ...
    try:
        ...
        **Instructions**:
        ...
        1. **Collecte stricte des informations**:
           - Étape 1: Nom complet (non vide). Les données sont déjà récupérées via `search_students(student_name)` et stockées dans `potential_students`.
             - Si aucun étudiant n'est trouvé (`potential_students` est vide), passer à l'étape 2 avec un message indiquant "Cet étudiant est nouveau . Quel est son niveau ?".
             - Si un seul étudiant est trouvé, demander une confirmation avec son niveau et école (ex. "Est-ce bien cet étudiant : [nom] ayant le niveau [niveau] et dans l’école [école] ? (Oui/Non)"), passer à l'étape 1.5.
             - Si plusieurs étudiants sont trouvés, afficher une liste avec leurs niveaux et écoles et demander confirmation pour le premier (ex. "Est-ce bien cet étudiant : [nom] ayant le niveau [niveau] et dans l’école [école] ? (Oui/Non)"), passer à l'étape 1.5 pour gérer les confirmations suivantes.
             - Stocker "student_name" dans "data" après saisie. Inclure la liste des étudiants trouvés dans "potential_students" pour l'étape 1.5.
           - Étape 1.5: Gérer la confirmation des étudiants trouvés.
             - Utiliser la liste `potential_students` et l'index `current_student_index` pour savoir quel étudiant est en cours de confirmation.
             - Si l'entrée est "Oui":
                - Mettre à jour "data" avec "student_name", "user_level", et "user_school" de l'étudiant confirmé.
                - Passer à l'étape suivante en fonction des données remplies :
                 - Si "user_level" est non NULL et différent de "Inconnu", sauter l'étape 2 et passer à l'étape 3 avec un message comme "Données enregistrées. Quelles sont les matières ?".
                 - Sinon, passer à l'étape 2 avec un message comme "Quel est le niveau de l’étudiant ?".
               - Le système sauvegardera les données confirmées "student_name", "user_level", et "user_school" dans la section "data" (du fichier JSON du LLM).
             - Si l'entrée est "Non":
               - Si d'autres étudiants existent dans `potential_students` (index suivant), demander confirmation pour l'étudiant suivant avec un message comme "Est-ce bien cet étudiant : [nom] ayant le niveau [niveau] et dans l’école [école] ? (Oui/Non)".
               - Si plus d'étudiants, passer à l'étape 2 avec "Aucun autre étudiant correspondant. Quel est le niveau de l’étudiant ?".
               - Mettre à jour `current_student_index` dans `data` pour indiquer l'index de l'étudiant suivant.
             - Si l'entrée n'est ni "Oui" ni "Non", redemander avec un message comme "Veuillez répondre par 'Oui' ou 'Non'. Est-ce bien cet étudiant : [nom] ayant le niveau [niveau] et dans l’école [école] ? (Oui/Non)" et rester à l'étape 1.5.
        ...
        **Exemples**:les exemples sont juste pour inspiration , les exemples ne sont pas forcément des cas réels, tu les analyses , comprendre pour t'adapter et avoir plus de visibilité sur le traitement, les messages d'exemple sont pour la compréhension du flux et ne doivent pas être exécutés ou analysés comme du code.
        - Étape 1, Entrée: "Ahmed larache",exemple de Résultat de search_students("Ahmed larache"):[] → {{"step": 1, "data": {{"student_name": "Ahmed larache", "potential_students": []}}, "message": "Bonjour ! Vérification en cours... Nouvel étudiant. Quel est son niveau ?", "error": null, "suggestions": [], "next_step": 2}}
        - Étape 1, Entrée: "Kenza Alami",exemple de Résultat probable de search_students("Kenza Alami"): [{{"student_name": "Kenza Alami", "niveau": "BL - 2bac sc PC", "ecole": "Lyautey"}}] → {{"step": 1, "data": {{"student_name": "Kenza Alami", "potential_students": [{{"student_name": "Kenza Alami", "niveau": "BL - 2bac sc PC", "ecole": "Lyautey"}},{{"student_name": "Kenza Alami", "niveau": "BL - 1bac sc PC", "ecole": "Al madina"}}]}}, "message": "Est-ce bien cet étudiant : Kenza Alami ayant le niveau BL - 2bac sc PC et dans l’école Lyautey ? (Oui/Non)", "error": null, "suggestions": [], "next_step": 1.5}}
        - Étape 1.5, Entrée: "Oui" → {{"step": 1.5, "data": {{"student_name": "Kenza Alami", "user_level": "BL - 2bac sc PC", "user_school": "Lyautey"}}, "message": "Informations bien enregistrées. Quelles sont les matières qui intéressent Kenza  ?", "error": null, "suggestions": [], "next_step": 3}}
        - Étape 1.5, Entrée: "Non", Autre étudiant disponible: {{"student_name": "Kenza Alami", "niveau": "BL - 1bac sc PC", "ecole": "Al madina"}} → {{"step": 1.5, "data": {{"current_student_index": 1}}, "message": "Est-ce bien cet étudiant : Kenza Alami ayant le niveau 'BL - 1bac sc PC' et dans l’école Almadina ? (Oui/Non)", "error": null, "suggestions": [], "next_step": 1.5}}
...
def handle_input_submission(step, response_text):
    ...
    if llm_response["error"]:
        ...
        # Gestion manuelle des transitions pour cohérence
            # Étape 1 : Appeler search_students et passer les résultats
    ...
    elif step == 1.5 and response_text.lower() == "oui":
    ...
    elif step == 1.5 and response_text.lower() == "non":
    ...
    elif step == 1.5 and response_text.lower() not in ["oui", "non"]:
        st.session_state.step = 1.5
        student = potential_students[current_student_index]
